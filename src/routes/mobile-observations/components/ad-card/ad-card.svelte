<script lang="ts">
	...
	async function _onDeleteConfirm() {
		if (!isShowingDeleteModal) return;
		await session.ads.requestUnindex(adData);
		onDeleteConfirmed?.();
		isShowingDeleteModal = false;
		isDeleted = true;
	}

	const CONFIRM_DELETE_TEXT = 'permanently delete';
...
<Dialog.Root bind:open={isShowingDeleteModal}>
	<Dialog.Content>
		...
		<div class="mx-auto h-96 w-64">
			<AdCardBody bind:adData={reactiveAdData} visible={intersecting} />
		</div>
		<div>
			Type "<strong>{CONFIRM_DELETE_TEXT}</strong>" in the box below to confirm.
			<Input bind:value={confirmationText} placeholder="permanently delete" class="mt-2" />
		</div>
		<div class="flex justify-end gap-2">
			<Dialog.Close>
				<Button variant="outline" size="sm" onclick={_onDeleteCancel}>Cancel</Button>
			</Dialog.Close>
			<Dialog.Close>
				<Button
					variant="destructive"
					size="sm"
					disabled={confirmationText !== CONFIRM_DELETE_TEXT}
					onclick={_onDeleteConfirm}>Delete</Button
				>
			</Dialog.Close>
		</div>
	</Dialog.Content>
...
<IntersectionObserverSvelte {element} threshold={0.25} once bind:intersecting>
	{#if !isDeleted}
		<div
			class={twMerge(
				'content-visibility-auto mb-4 flex w-fit break-inside-avoid flex-col gap-2 rounded border bg-zinc-50 p-4 transition-all hover:shadow-lg dark:bg-zinc-900 dark:shadow-zinc-800',
					...
					<div class="flex items-center gap-2">
						...
						<div class="flex items-center gap-1">
							...
							<Button
								...
								class="text-destructive size-fit p-1.5"
								size="icon"
								onclick={_onDeleteStart}
							>
								<TrashIcon />
							</Button>
						</div>
	...
	{#if isDeleted}
		<div class="flex size-full items-center justify-center">
			<span class="text-center text-lg font-medium text-red-600">This ad has been deleted.</span>
		</div>
	{/if}
